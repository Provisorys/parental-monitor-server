...
// --- Rotas HTTP (MANTIDAS EXATAMENTE COMO NO SEU CÓDIGO ORIGINAL) ---
...
// Rota para obter IDs de filhos associados a um pai (MANTIDA COM :parentId na URL)
...
wssAudio.on('connection', (ws, req) => {
    ...
    ws.on('message', async (message) => {
        ...
        try {
            ...
            // Identificação do cliente (pai ou filho) para o canal de ÁUDIO
            if (parsedMessage.type === 'parent_id' && parsedMessage.id) {
                ws.clientId = parsedMessage.id;
                ws.clientType = 'parent';
                wsClientsMap.set(parsedMessage.id, ws); // O pai também está no mapa principal
                console.log(`[WS-AUDIO] Pai conectado e identificado: ${ws.clientId}`);
            } else if (parsedMessage.type === 'child_id' && parsedMessage.id) {
                ws.clientId = parsedMessage.id;
                ws.clientType = 'child';
                wsClientsMap.set(parsedMessage.id, ws); // O filho também está no mapa principal
                activeChildWebSockets.set(parsedMessage.id, ws); // Adiciona ao mapa de filhos ativos
                console.log(`[WS-AUDIO] Filho conectado e identificado: ${ws.clientId}`);
                // Não notificar pais aqui, pois esta é a conexão de ÁUDIO, não de STATUS.
            } else if (!ws.clientId) {
                console.warn(`[WS-AUDIO] Mensagem recebida de cliente não identificado: ${messageString}`);
                ws.send(JSON.stringify({ type: 'error', message: 'Cliente não identificado. Por favor, envie seu ID.' }));
                return;
            }

            // Lógica específica para o canal de ÁUDIO
            if (ws.clientType === 'child') {
                if (parsedMessage.type === 'audio_stream_data' && parsedMessage.data) {
                    // Recebe dados de áudio em Base64 do filho e envia para o pai ouvindo
                    const parentWs = parentListeningSockets.get(ws.clientId); // Pega o WS do pai (conexão de comando)
                    if (parentWs && parentWs.readyState === WebSocket.OPEN) {
                        parentWs.send(JSON.stringify({
                            type: 'audio_stream_data',
                            childId: ws.clientId,
                            data: parsedMessage.data // Dados de áudio em Base64
                        }));
                    }
                }
            }
            // Outras mensagens de controle para a conexão de áudio podem ser adicionadas aqui se necessário
        } catch (e) {
            console.error(`[WS-AUDIO] Erro ao processar mensagem JSON: ${e.message}, Mensagem original: ${messageString}`);
    ...
    ws.on('close', (code, reason) => {
        ...
        if (ws.clientId) {
            // Se for um filho, remover do mapa de filhos ativos
            if (ws.clientType === 'child') {
                activeChildWebSockets.delete(ws.clientId);
                // Não notificar pais sobre offline aqui, o WS-COMMANDS fará isso.
                // Se o filho estava enviando áudio, parar de enviar
                parentListeningSockets.delete(ws.clientId); // Remover a escuta para o filho que desconectou
            }
            // Não remove do wsClientsMap aqui, pois o cliente ainda pode estar conectado via WS-COMMANDS
        }
    });
...
wssCommands.on('connection', (ws, req) => {
        ...
        try {
            ...
            if (parsedMessage.type === 'parent_id' && parsedMessage.id) {
                ...
                wsClientsMap.forEach((clientWs) => {
                    ...
                    if (clientWs.clientType === 'parent' && clientWs.readyState === WebSocket.OPEN && clientWs.url === '/ws-commands') {
            ...
            if (ws.clientType === 'parent') {
                if (parsedMessage.type === 'request_current_location' && parsedMessage.childId) {
                    ...
                    const targetChildWs = activeChildWebSockets.get(parsedMessage.childId); // Usa o mapa de filhos ativos
                    if (targetChildWs && targetChildWs.readyState === WebSocket.OPEN) {
                        // Registra o pai para receber o stream na conexão de COMANDOS
                        parentListeningSockets.set(parsedMessage.childId, ws);
                    ...
                    const targetChildWs = activeChildWebSockets.get(parsedMessage.childId); // Usa o mapa de filhos ativos
                    if (targetChildWs && targetChildWs.readyState === WebSocket.OPEN) {
                        parentListeningSockets.delete(parsedMessage.childId); // Remove o pai do mapa de escuta
                ...
                // A lógica de audio_stream_data NÃO FICA AQUI, ela fica no wssAudio
            }
        } catch (e) {
    ...
    ws.on('close', (code, reason) => {
        console.log(`[WS-COMMANDS] Cliente desconectado: ${ws.clientId || 'desconhecido'}. Código: ${code}, Razão: ${reason}`);
        if (ws.clientId) {
            // Se for um filho e a conexão de comando for fechada
            if (ws.clientType === 'child') {
            ...
            // Remove o cliente do mapa principal wsClientsMap apenas se esta for a última conexão ativa do cliente.
            // (Para evitar remover se ele ainda tiver uma conexão WS-AUDIO aberta, por exemplo)
            // No entanto, para simplicidade e seguindo o modelo original, ele é removido do wsClientsMap.
            wsClientsMap.delete(ws.clientId);

            // Remover do parentListeningSockets se este filho estava sendo escutado
            parentListeningSockets.delete(ws.clientId); // Isso é importante para parar de tentar enviar áudio para pais que não estão mais ouvindo
        }
    });

    ws.on('error', (error) => {
        console.error(`[WS-COMMANDS] Erro no WebSocket para cliente ${ws.clientId || 'desconhecido'}:`, error);
    });
});


// Tratamento de rotas não encontradas (MANTIDO DO SEU ORIGINAL)
app.use((req, res) => {
    console.warn(`[HTTP_ERROR] Rota não encontrada: ${req.method} ${req.url}`); // Log
    res.status(404).send('Rota não encontrada');
});

// Tratamento de erros gerais (MANTIDO DO SEU ORIGINAL)
